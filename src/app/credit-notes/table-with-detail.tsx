...
import { deleteCreditNoteAction } from "@/app/credit-notes/actions";
...
export function CreditNoteTableWithDetail({
  rows,
  searchSlot,
  topActions,
}: {
  ...
  function handleDelete() {
    ...
    if (
      !confirm(
        `ต้องการลบใบลดหนี้ ${selected.docNo} (${selected.customerName ?? ""}) ออกจากระบบ?\nการลบนี้ไม่สามารถย้อนกลับได้`,
      )
    )
      return;
    startTransition(async () => {
      const res = await deleteCreditNoteAction(selected.id);
      if (res?.error) {
    ...
    <div className="space-y-3">
      ...
      {(topActions || rows.length > 0) && (
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={
              selected
                ? `ลบใบลดหนี้ ${selected.docNo} ออกจากระบบ`
                : "เลือกแถวในตารางก่อน"
            }
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">รายการใบลดหนี้ในระบบ</CardTitle>
        </CardHeader>
            ...
            <table className="w-full text-sm">
              <thead className="border-b bg-zinc-50 text-left text-xs text-zinc-500">
                <tr>
                  ...
                  <th className="px-4 py-2.5 font-medium">อ้างอิงใบกำกับ</th>
      ...
      {selected && <DetailPanel cn={selected} />}
    </div>
  );
}

function DetailPanel({ cn }: { cn: CreditNoteRow }) {
  const addrLine = (cn.customerAddress ?? "").replace(/\n/g, " ");
    ...
    <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_360px]">
      ...
      <div className="overflow-hidden rounded border-2 border-rose-400 bg-white text-[12px]">
        <div className="grid grid-cols-[1fr_1fr] gap-x-3 gap-y-1 border-b border-rose-200 bg-rose-50 px-3 py-2">
          <div>
            <div className="text-[10px] text-rose-700">CreditNote :</div>
            <div className="font-mono font-bold text-rose-900">{cn.docNo}</div>
          </div>
          <div>
            <div className="text-[10px] text-rose-700">อ้างอิงใบกำกับ</div>
            <div className="font-mono font-medium text-zinc-800">
              {cn.referenceInvoiceNo ?? "-"}
            </div>
          </div>
          <div>
            <div className="text-[10px] text-rose-700">วันที่ใบลดหนี้</div>
            <div className="font-mono text-zinc-800">
              {formatThaiDateShort(cn.docDate)}
            </div>
          </div>
          <div>
            ...
            <div className="font-mono text-zinc-800">
              {cn.referenceInvoiceDate
                ? formatThaiDateShort(cn.referenceInvoiceDate)
                : "-"}
            </div>
          </div>
          <div>
            <div className="text-[10px] text-rose-700">พนักงานขาย</div>
            <div className="text-zinc-800">{cn.salemanName ?? "-"}</div>
          </div>
        </div>
        <div className="space-y-1 px-3 py-2">
          <div className="flex justify-between text-zinc-700">
            ...
            <span className="tabular-nums">
              {formatMoney(cn.amountBeforeVat)}
            </span>
          </div>
          <div className="flex justify-between text-zinc-700">
            <span>ภาษีมูลค่าเพิ่ม</span>
            <span className="tabular-nums">{formatMoney(cn.vatAmount)}</span>
          </div>
          <div className="flex justify-between border-t pt-1 text-base font-bold text-rose-700">
            <span>รวมเงินทั้งสิ้น</span>
            <span className="tabular-nums">{formatMoney(cn.total)}</span>
          </div>
        </div>
      </div>
    </div>
